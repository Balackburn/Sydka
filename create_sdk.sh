...
################################################################################
# create_sdk.sh - Automated iOS SDK Patcher
#
...
# Path to the dynamic Xcode↔iOS SDK map (generated by fetch.py)
SDK_MAP_FILE="${SCRIPT_DIR}/xcode_ios_sdk_map.json"
...
# Ensure xcode_ios_sdk_map.json is available locally.
...
ensure_sdk_map() {
    ...
    log_warn "xcode_ios_sdk_map.json not found at: ${SDK_MAP_FILE}"

    # Try to generate it on-the-fly using fetch.py (requires Python + pip deps)
    local fetch_script="${SCRIPT_DIR}/fetch.py"
    if [ -f "${fetch_script}" ] && command -v python3 &>/dev/null; then
        log_info "Attempting to generate xcode_ios_sdk_map.json via fetch.py …"
        if python3 -m pip install --quiet requests beautifulsoup4 2>/dev/null && \
           python3 "${fetch_script}" --output "${SDK_MAP_FILE}" 2>/dev/null; then
...
# Reads from xcode_ios_sdk_map.json (generated by fetch.py).
...
candidates = [
    ...
    if r.get("ios_sdk") == ios_sdk and r.get("xcode_version")
]
...
KNOWN iOS SDK VERSIONS (read from xcode_ios_sdk_map.json — run fetch.py to update):
$(all_known_ios_sdks 2>/dev/null | tr '\n' '  ' || echo "  (run fetch.py to generate the map)")
EOF
}
...
main() {
    ...
    # Ensure the Xcode↔iOS SDK map is available (may auto-generate via fetch.py)
    ensure_sdk_map
